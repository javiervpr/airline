...
@Component
public class PassengerPayCompleteHandler
  implements Command.Handler<PassengerPayCompleteQuery, List<PassangerDto>> {
  ...
  public List<PassangerDto> handle(PassengerPayCompleteQuery command) {
    try {
      List<Ticket> tickets =
        this.ticketRepository.getByFlightId(UUID.fromString(command.flightId));
      return tickets
        .stream()

...
@Component
public class TicketSyncHandler
  implements Command.Handler<TicketSyncCommand, UUID> {

  private final TicketRepository ticketRepository;
  private final PassangerRepository passangerRepository;

  public TicketSyncHandler(
  ...
  public UUID handle(TicketSyncCommand command) {
    ...
    try {
      Passanger passanger = passangerRepository.get(
        UUID.fromString(paymentCompleteDto.body.passanger.id)
      );

...
@Component
public class GetSeatsByFlightHandler implements Command.Handler<GetSeatsByFlightQuery, List<SeatDto>> {
    ...
    public List<SeatDto> handle(GetSeatsByFlightQuery command) {
        try {
            List<Seat> seats = seatRepository.findByFlightId(UUID.fromString(command.flightId));
            return seats.stream()
                    .map(SeatMapper::from)
                    .toList();
